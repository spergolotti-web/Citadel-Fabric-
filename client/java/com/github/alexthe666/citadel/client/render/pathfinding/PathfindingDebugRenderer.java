...
public class PathfindingDebugRenderer {
    ...
    private static void debugDrawNode(final MNode n, final int argbColor, final WorldEventContext ctx) {
        ...
        WorldRenderMacros.renderBox(ctx.submitCollector, ctx.poseStack, BlockPos.ZERO, BlockPos.ZERO, argbColor);
        ...
        if (n.parent != null) {
            ...
            ctx.submitCollector.submitCustomGeometry(ctx.poseStack, WorldRenderMacros.LINES, (pose, buffer) -> {
    ...
    private static void renderDebugText(@NotNull final MNode n, final WorldEventContext ctx) {
        ...
        WorldRenderMacros.renderFillRectangle(ctx.submitCollector, ctx.poseStack, -i - 1, -5, 0, 2 * i + 2, 17, 0x7f000000);

        ctx.poseStack.translate(0.0F, -5F, -0.1F);
        ctx.submitCollector.submitText(ctx.poseStack, -fontrenderer.width(s1) / 2.0f, 1, Component.literal(s1).getVisualOrderText(), false, Font.DisplayMode.NORMAL, 0xFFFFFFFF, 0, 15728880, 0);
        ctx.poseStack.translate(0.0F, 8F, -0.1F);
        ctx.submitCollector.submitText(ctx.poseStack, -fontrenderer.width(s2) / 2.0f, 1, Component.literal(s2).getVisualOrderText(), false, Font.DisplayMode.NORMAL, 0xFFFFFFFF, 0, 15728880, 0);
