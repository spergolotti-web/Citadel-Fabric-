...
public abstract class TickRateModifier {
    ...
    public TickRateModifier(CompoundTag tag) {
        this.type = TickRateModifierType.fromId(tag.getIntOr("TickRateType", 0));
        this.maxDuration = tag.getFloatOr("MaxDuration", 0.0F);
        this.duration = tag.getFloatOr("Duration", 0.0F);
        this.tickRateMultiplier = tag.getFloatOr("SpeedMultiplier", 0.0F);
    }
    ...
    public static TickRateModifier fromTag(CompoundTag tag) {
        TickRateModifierType typeFromNbt = TickRateModifierType.fromId(tag.getIntOr("TickRateType", 0));
        try {
