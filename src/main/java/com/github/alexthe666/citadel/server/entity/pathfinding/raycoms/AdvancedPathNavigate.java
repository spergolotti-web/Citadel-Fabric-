...
import com.github.alexthe666.citadel.mixin.NodeEvaluatorAccessor;
...
public class AdvancedPathNavigate extends AbstractAdvancedPathNavigate {
    ...
    @Override
    public void tick() {
        NodeEvaluatorAccessor accessor = (NodeEvaluatorAccessor) this.nodeEvaluator;
        if (overrideDefaultDimensions) {
            accessor.citadel$setEntityWidth(Mth.floor(width + 1.0F));
            accessor.citadel$setEntityHeight(Mth.floor(height + 1.0F));
            accessor.citadel$setEntityDepth(Mth.floor(width + 1.0F));
        } else {
            accessor.citadel$setEntityWidth(Mth.floor(ourEntity.getBbWidth() + 1.0F));
            accessor.citadel$setEntityHeight(Mth.floor(ourEntity.getBbHeight() + 1.0F));
            accessor.citadel$setEntityDepth(Mth.floor(ourEntity.getBbWidth() + 1.0F));
        }
    ...
    private boolean handleLadders(int oldIndex) {
        ...
        if (!this.isDone()) {
            ...
            if (pEx.isOnLadder() && pExNext != null && (pEx.y != pExNext.y || mob.getY() > pEx.y) && com.github.alexthe666.citadel.server.entity.collision.CustomCollisionsNodeProcessor.isLadder(level.getBlockState(pos), level, pos)) {
    ...
    private boolean handlePathPointOnLadder(final PathPointExtended pEx) {
        ...
        if (vec3.distanceToSqr(ourEntity.getX(), vec3.y, ourEntity.getZ()) < 0.6 && Math.abs(vec3.y - entityPos.getY()) <= 2.0) {
            ...
            if (newSpeed > 0) {
                ...
                if (com.github.alexthe666.citadel.server.entity.collision.CustomCollisionsNodeProcessor.isLadder(level.getBlockState(entityPos.below()), level, entityPos.below())) {
