    ...
    public static class CitadelThreadFactory implements ThreadFactory {
        ...
        public Thread newThread(final @NotNull Runnable runnable) throws RuntimeException {
            ...
            if (server == null || server.isSameThread()) {
                ...
                classLoader = CompletableFuture.supplyAsync(() -> Thread.currentThread().getContextClassLoader(), server).orTimeout(10, TimeUnit.SECONDS).exceptionally((ex) -> {
                    throw new RuntimeException(String.format("Couldn't join threads within timeout range. Tried joining '%s' on '%s'", Thread.currentThread().getName(), server.name()));
                }).join();
            }
