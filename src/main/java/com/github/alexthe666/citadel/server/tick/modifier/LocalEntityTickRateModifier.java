...
public class LocalEntityTickRateModifier extends LocalTickRateModifier {
    ...
    public LocalEntityTickRateModifier(CompoundTag tag) {
        ...
        EntityType type = BuiltInRegistries.ENTITY_TYPE.getOptional(ResourceLocation.parse(tag.getString("EntityType"))).orElse(null);
    ...
    public boolean isEntityValid(Level level) {
        Entity entity = level.getEntity(this.entityId);
        return entity != null && entity.level() != null && entity.getType().equals(expectedEntityType) && entity.isAlive() && (!(entity instanceof IModifiesTime) || ((IModifiesTime) entity).isTimeModificationValid(this));
    }
