...
public class Tetris {
        ...
        for (int tries = 0; tries < 5; tries++) {
            if (allRegisteredBlocks.length > 1) {
                ...
                try {
                    ...
                    if (blockModel != null && !block.is(Blocks.GLOWSTONE) && !blockModel.isCustomRenderer()) {
                        randomState = block;
                        break;
                    }
    ...
    private void renderBlockState(BlockState state, float offsetX, float offsetY, float size) {
        ...
        float f = size * 0.5F;
        float z = 80.0F;
        bufferbuilder.addVertex(-f + offsetX, f + offsetY, z).setUv(sprite.getU0(), sprite.getV1());
        bufferbuilder.addVertex(f + offsetX, f + offsetY, z).setUv(sprite.getU1(), sprite.getV1());
        bufferbuilder.addVertex(f + offsetX, -f + offsetY, z).setUv(sprite.getU1(), sprite.getV0());
        bufferbuilder.addVertex(-f + offsetX, -f + offsetY, z).setUv(sprite.getU0(), sprite.getV0());
