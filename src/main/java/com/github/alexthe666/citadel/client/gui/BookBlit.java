...
public class BookBlit {
    ...
    private static void blitWithColor(GuiGraphics guiGraphics, ResourceLocation texture, int startX, int endX, int startY, int endY, int zLevel, float u0, float u1, float v0, float v1, int r, int g, int b, int a) {
        ...
        v.set((float) startX, (float) startY, (float) zLevel, 1f); v.mul(matrix4f); bufferbuilder.addVertex(v.x(), v.y(), v.z()).setUv(u0, v0).setColor(r, g, b, a);
        v.set((float) startX, (float) endY, (float) zLevel, 1f); v.mul(matrix4f); bufferbuilder.addVertex(v.x(), v.y(), v.z()).setUv(u0, v1).setColor(r, g, b, a);
        v.set((float) endX, (float) endY, (float) zLevel, 1f); v.mul(matrix4f); bufferbuilder.addVertex(v.x(), v.y(), v.z()).setUv(u1, v1).setColor(r, g, b, a);
        v.set((float) endX, (float) startY, (float) zLevel, 1f); v.mul(matrix4f); bufferbuilder.addVertex(v.x(), v.y(), v.z()).setUv(u1, v0).setColor(r, g, b, a);
