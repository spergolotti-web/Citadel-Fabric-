    ...
    private void endKeyframe(boolean stationary) {
        ...
        if (animationTick >= this.prevTempTick && animationTick < this.tempTick) {
            if (stationary) {
                ...
                float frameTime = Minecraft.getInstance().isPaused() ? 0.0F : ((com.github.alexthe666.citadel.mixin.client.MinecraftAccessor) Minecraft.getInstance()).citadel$getTimer().getGameTimeDeltaPartialTick(false);
